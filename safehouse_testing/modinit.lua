...
local function init( modApi )
	...
	local getCorpData_old = serverdefs.getCorpData	
	serverdefs.getCorpData = function( situation, ... )
		log:write("LOG custom corpData")
		log:write(util.stringize(situation,3))
		if situation.name == "level_test" then
			situation.corpName = "sankaku"
			-- situation.mapLocation = 
		end
		return getCorpData_old(situation, ... )
	end
	
	
	
   local worldgen = include( scriptPath .. "/worldgen" )
...
local function load( modApi, options, params )
	...
	local serverdefs = include( "modules/serverdefs" )
	-- local createNewSituation_old = serverdefs.createNewSituation
	-- serverdefs.createNewSituation = function(campaign, gen, tags, difficulty, ... )
		-- log:write("custom createNewSituation")
		-- local situation = createNewSituation_old(campaign, gen, tags, difficulty, ... )
		-- if situation and (situation.name == "level_test") then
			-- log:write("LOG modifying situation corp")
			-- situation.corpName = "agency"
		-- end
		-- return situation	
	-- end
	
	-- local chooseSituation_old = serverdefs.chooseSituation
	-- serverdefs.chooseSituation = function( campaign, tags, gen, ... )
		-- local situation = chooseSituation_old( campaign, tags, gen, ... )
		-- if 
...
local widget = {
		...
		local map_colour_normal = {61/255,13/255,47/255,1}
		local map_colour_highlight = {166/255,66/255,206/255,1}
		local map_colour_unhighlight =  {34/255,57/255,56/255,.5}
		modApi:addCorpMap( "agency", widget, map_colour_normal, map_colour_highlight, map_colour_unhighlight, true )
		
	-- local simdefs = include( "sim/simdefs" )
	-- local corp = "agency"
	-- if not simdefs.ENABLED_CORPS[corp] then
		-- simdefs.ENABLED_CORPS[corp] = true
		-- table.insert(simdefs.CORP_NAMES,corp)
		-- table.insert(simdefs.DEFAULT_MISSION_TAGS,corp)
	-- end
	
	log:write("LOG corps")
	log:write(util.stringize(serverdefs.CORP_DATA,4))
	...
	include( scriptPath .. "/missions/safehouse_script" )
	
	include( scriptPath .. "/lvl_safehouse_test" )
